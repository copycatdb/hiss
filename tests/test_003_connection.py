...
import pytest
import time
from hiss import connect, Connection, pooling, SQL_CHAR, SQL_WCHAR
import threading
# Import all exception classes for testing
from hiss.exceptions import (
    Warning,
    Error,
    InterfaceError,
    DatabaseError,
...
from hiss.constants import ConstantsDDBC
...
def clean_connection_state(db_connection):
    ...
    try:
        ...
        pass  # Ignore errors during cleanup
from hiss.constants import GetInfoConstants as sql_const
...
def test_setencoding_constants_access():
    ...
    assert hasattr(hiss, 'SQL_CHAR'), "SQL_CHAR constant should be available"
    assert hasattr(hiss, 'SQL_WCHAR'), "SQL_WCHAR constant should be available"
    assert hiss.SQL_CHAR == 1, "SQL_CHAR should have value 1"
    assert hiss.SQL_WCHAR == -8, "SQL_WCHAR should have value -8"
...
def test_setencoding_with_constants(db_connection):
    ...
    db_connection.setencoding(encoding='utf-8', ctype=hiss.SQL_CHAR)
    settings = db_connection.getencoding()
    assert settings['ctype'] == hiss.SQL_CHAR, "Should accept SQL_CHAR constant"
    
    # Test with SQL_WCHAR constant
    db_connection.setencoding(encoding='utf-16le', ctype=hiss.SQL_WCHAR)
    settings = db_connection.getencoding()
    assert settings['ctype'] == hiss.SQL_WCHAR, "Should accept SQL_WCHAR constant"
...
def test_setdecoding_default_settings(db_connection):
    ...
    sql_char_settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert sql_char_settings['encoding'] == 'utf-8', "Default SQL_CHAR encoding should be utf-8"
    assert sql_char_settings['ctype'] == hiss.SQL_CHAR, "Default SQL_CHAR ctype should be SQL_CHAR"
    
    # Check SQL_WCHAR defaults
    sql_wchar_settings = db_connection.getdecoding(hiss.SQL_WCHAR)
    assert sql_wchar_settings['encoding'] == 'utf-16le', "Default SQL_WCHAR encoding should be utf-16le"
    assert sql_wchar_settings['ctype'] == hiss.SQL_WCHAR, "Default SQL_WCHAR ctype should be SQL_WCHAR"
    
    # Check SQL_WMETADATA defaults
    sql_wmetadata_settings = db_connection.getdecoding(hiss.SQL_WMETADATA)
    assert sql_wmetadata_settings['encoding'] == 'utf-16le', "Default SQL_WMETADATA encoding should be utf-16le"
    assert sql_wmetadata_settings['ctype'] == hiss.SQL_WCHAR, "Default SQL_WMETADATA ctype should be SQL_WCHAR"
...
def test_setdecoding_basic_functionality(db_connection):
    ...
    db_connection.setdecoding(hiss.SQL_CHAR, encoding='latin-1')
    settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert settings['encoding'] == 'latin-1', "SQL_CHAR encoding should be set to latin-1"
    assert settings['ctype'] == hiss.SQL_CHAR, "SQL_CHAR ctype should default to SQL_CHAR for latin-1"
    
    # Test setting SQL_WCHAR decoding
    db_connection.setdecoding(hiss.SQL_WCHAR, encoding='utf-16be')
    settings = db_connection.getdecoding(hiss.SQL_WCHAR)
    assert settings['encoding'] == 'utf-16be', "SQL_WCHAR encoding should be set to utf-16be"
    assert settings['ctype'] == hiss.SQL_WCHAR, "SQL_WCHAR ctype should default to SQL_WCHAR for utf-16be"
    
    # Test setting SQL_WMETADATA decoding
    db_connection.setdecoding(hiss.SQL_WMETADATA, encoding='utf-16le')
    settings = db_connection.getdecoding(hiss.SQL_WMETADATA)
    assert settings['encoding'] == 'utf-16le', "SQL_WMETADATA encoding should be set to utf-16le"
    assert settings['ctype'] == hiss.SQL_WCHAR, "SQL_WMETADATA ctype should default to SQL_WCHAR"
...
def test_setdecoding_automatic_ctype_detection(db_connection):
    ...
    utf16_encodings = ['utf-16', 'utf-16le', 'utf-16be']
    for encoding in utf16_encodings:
        db_connection.setdecoding(hiss.SQL_CHAR, encoding=encoding)
        settings = db_connection.getdecoding(hiss.SQL_CHAR)
        assert settings['ctype'] == hiss.SQL_WCHAR, f"SQL_CHAR with {encoding} should auto-detect SQL_WCHAR ctype"
    
    # Other encodings should default to SQL_CHAR
    other_encodings = ['utf-8', 'latin-1', 'ascii', 'cp1252']
    for encoding in other_encodings:
        db_connection.setdecoding(hiss.SQL_WCHAR, encoding=encoding)
        settings = db_connection.getdecoding(hiss.SQL_WCHAR)
        assert settings['ctype'] == hiss.SQL_CHAR, f"SQL_WCHAR with {encoding} should auto-detect SQL_CHAR ctype"

def test_setdecoding_explicit_ctype_override(db_connection):
    """Test that explicit ctype parameter overrides automatic detection."""
    
    # Set SQL_CHAR with UTF-8 encoding but explicit SQL_WCHAR ctype
    db_connection.setdecoding(hiss.SQL_CHAR, encoding='utf-8', ctype=hiss.SQL_WCHAR)
    settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert settings['encoding'] == 'utf-8', "Encoding should be utf-8"
    assert settings['ctype'] == hiss.SQL_WCHAR, "ctype should be SQL_WCHAR when explicitly set"
    
    # Set SQL_WCHAR with UTF-16LE encoding but explicit SQL_CHAR ctype
    db_connection.setdecoding(hiss.SQL_WCHAR, encoding='utf-16le', ctype=hiss.SQL_CHAR)
    settings = db_connection.getdecoding(hiss.SQL_WCHAR)
    assert settings['encoding'] == 'utf-16le', "Encoding should be utf-16le"
    assert settings['ctype'] == hiss.SQL_CHAR, "ctype should be SQL_CHAR when explicitly set"
...
def test_setdecoding_none_parameters(db_connection):
    ...
    db_connection.setdecoding(hiss.SQL_CHAR, encoding=None)
    settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert settings['encoding'] == 'utf-8', "SQL_CHAR with encoding=None should use utf-8 default"
    assert settings['ctype'] == hiss.SQL_CHAR, "ctype should be SQL_CHAR for utf-8"
    
    # Test SQL_WCHAR with encoding=None (should use utf-16le default)
    db_connection.setdecoding(hiss.SQL_WCHAR, encoding=None)
    settings = db_connection.getdecoding(hiss.SQL_WCHAR)
    assert settings['encoding'] == 'utf-16le', "SQL_WCHAR with encoding=None should use utf-16le default"
    assert settings['ctype'] == hiss.SQL_WCHAR, "ctype should be SQL_WCHAR for utf-16le"
    
    # Test with both parameters None
    db_connection.setdecoding(hiss.SQL_CHAR, encoding=None, ctype=None)
    settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert settings['encoding'] == 'utf-8', "SQL_CHAR with both None should use utf-8 default"
    assert settings['ctype'] == hiss.SQL_CHAR, "ctype should default to SQL_CHAR"
...
def test_setdecoding_invalid_encoding(db_connection):
    ...
    with pytest.raises(ProgrammingError) as exc_info:
        db_connection.setdecoding(hiss.SQL_CHAR, encoding='invalid-encoding-name')
...
def test_setdecoding_invalid_ctype(db_connection):
    ...
    with pytest.raises(ProgrammingError) as exc_info:
        db_connection.setdecoding(hiss.SQL_CHAR, encoding='utf-8', ctype=999)
...
def test_setdecoding_closed_connection(conn_str):
    ...
    with pytest.raises(InterfaceError) as exc_info:
        temp_conn.setdecoding(hiss.SQL_CHAR, encoding='utf-8')
...
def test_setdecoding_constants_access():
    ...
    assert hasattr(hiss, 'SQL_CHAR'), "SQL_CHAR constant should be available"
    assert hasattr(hiss, 'SQL_WCHAR'), "SQL_WCHAR constant should be available"
    assert hasattr(hiss, 'SQL_WMETADATA'), "SQL_WMETADATA constant should be available"
    
    assert hiss.SQL_CHAR == 1, "SQL_CHAR should have value 1"
    assert hiss.SQL_WCHAR == -8, "SQL_WCHAR should have value -8"
    assert hiss.SQL_WMETADATA == -99, "SQL_WMETADATA should have value -99"

def test_setdecoding_with_constants(db_connection):
    """Test setdecoding using module constants."""
    
    # Test with SQL_CHAR constant
    db_connection.setdecoding(hiss.SQL_CHAR, encoding='utf-8', ctype=hiss.SQL_CHAR)
    settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert settings['ctype'] == hiss.SQL_CHAR, "Should accept SQL_CHAR constant"
    
    # Test with SQL_WCHAR constant
    db_connection.setdecoding(hiss.SQL_WCHAR, encoding='utf-16le', ctype=hiss.SQL_WCHAR)
    settings = db_connection.getdecoding(hiss.SQL_WCHAR)
    assert settings['ctype'] == hiss.SQL_WCHAR, "Should accept SQL_WCHAR constant"
    
    # Test with SQL_WMETADATA constant
    db_connection.setdecoding(hiss.SQL_WMETADATA, encoding='utf-16be')
    settings = db_connection.getdecoding(hiss.SQL_WMETADATA)
...
def test_setdecoding_case_insensitive_encoding(db_connection):
    ...
    # Test various case formats
    db_connection.setdecoding(hiss.SQL_CHAR, encoding='UTF-8')
    settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert settings['encoding'] == 'utf-8', "Encoding should be normalized to lowercase"
    
    db_connection.setdecoding(hiss.SQL_WCHAR, encoding='Utf-16LE')
    settings = db_connection.getdecoding(hiss.SQL_WCHAR)
...
def test_setdecoding_override_previous(db_connection):
    ...
    assert settings['ctype'] == hiss.SQL_CHAR, "Initial ctype should be SQL_CHAR"
    
    # Override with different settings
    db_connection.setdecoding(hiss.SQL_CHAR, encoding='latin-1', ctype=hiss.SQL_WCHAR)
    settings = db_connection.getdecoding(hiss.SQL_CHAR)
    assert settings['encoding'] == 'latin-1', "Encoding should be overridden to latin-1"
    assert settings['ctype'] == hiss.SQL_WCHAR, "ctype should be overridden to SQL_WCHAR"
...
def test_getdecoding_closed_connection(conn_str):
    ...
    with pytest.raises(InterfaceError) as exc_info:
        temp_conn.getdecoding(hiss.SQL_CHAR)
...
def test_getdecoding_returns_copy(db_connection):
    ...
    # Get settings twice
    settings1 = db_connection.getdecoding(hiss.SQL_CHAR)
    settings2 = db_connection.getdecoding(hiss.SQL_CHAR)
...
def test_setdecoding_getdecoding_consistency(db_connection):
    ...
    test_cases = [
        (hiss.SQL_CHAR, 'utf-8', hiss.SQL_CHAR),
        (hiss.SQL_CHAR, 'utf-16le', hiss.SQL_WCHAR),
        (hiss.SQL_WCHAR, 'latin-1', hiss.SQL_CHAR),
        (hiss.SQL_WCHAR, 'utf-16be', hiss.SQL_WCHAR),
        (hiss.SQL_WMETADATA, 'utf-16le', hiss.SQL_WCHAR),
    ]
...
def test_setdecoding_persistence_across_cursors(db_connection):
    ...
    db_connection.setdecoding(hiss.SQL_CHAR, encoding='latin-1', ctype=hiss.SQL_CHAR)
    db_connection.setdecoding(hiss.SQL_WCHAR, encoding='utf-16be', ctype=hiss.SQL_WCHAR)
    
    # Create cursors and verify settings persist
    cursor1 = db_connection.cursor()
    char_settings1 = db_connection.getdecoding(hiss.SQL_CHAR)
    wchar_settings1 = db_connection.getdecoding(hiss.SQL_WCHAR)
    
    cursor2 = db_connection.cursor()
    char_settings2 = db_connection.getdecoding(hiss.SQL_CHAR)
    wchar_settings2 = db_connection.getdecoding(hiss.SQL_WCHAR)
...
def test_setdecoding_all_sql_types_independently(conn_str):
    ...
    try:
        ...
        test_configs = [
            (hiss.SQL_CHAR, 'ascii', hiss.SQL_CHAR),
            (hiss.SQL_WCHAR, 'utf-16le', hiss.SQL_WCHAR),
            (hiss.SQL_WMETADATA, 'utf-16be', hiss.SQL_WCHAR),
        ]
...
def test_setdecoding_security_logging(db_connection):
    ...
    test_cases = [
        ...
        (hiss.SQL_CHAR, 'invalid-encoding', None),  # Invalid encoding
        (hiss.SQL_CHAR, 'utf-8', 999),  # Invalid ctype
    ]
