...
#     """Test setinputsizes"""
#     sizes = [(hiss.ConstantsDDBC.SQL_INTEGER, 10), (hiss.ConstantsDDBC.SQL_VARCHAR, 255)]
#     cursor.setinputsizes(sizes)

# def test_setoutputsize(cursor):
#     """Test setoutputsize"""
#     cursor.setoutputsize(10, hiss.ConstantsDDBC.SQL_INTEGER)
...
def test_lowercase_setting_after_cursor_creation(cursor, db_connection):
    ...
    original_lowercase = hiss.lowercase
    try:
        # Create table and execute with lowercase=False
        hiss.lowercase = False
        ...
        hiss.lowercase = original_lowercase
        try:
...
def test_rownumber_warning_logged(cursor, db_connection):
    ...
    import logging
    from hiss.helpers import get_logger
...
def test_scroll_backward_not_supported(cursor, db_connection):
    ...
    from hiss.exceptions import NotSupportedError
    try:
...
def test_cursor_skip_invalid_arguments(cursor, db_connection):
    ...
    from hiss.exceptions import ProgrammingError, NotSupportedError
...
def test_decimal_separator_with_multiple_values(cursor, db_connection):
    ...
    original_separator = hiss.getDecimalSeparator()
    ...
    try:
        ...
        hiss.setDecimalSeparator(original_separator)
...
def test_decimal_separator_basic_functionality():
    ...
    try:
        # Test default value
        assert hiss.getDecimalSeparator() == '.', "Default decimal separator should be '.'"
        
        # Test setting to comma
        hiss.setDecimalSeparator(',')
        assert hiss.getDecimalSeparator() == ',', "Decimal separator should be ',' after setting"
        ...
        # Test invalid inputs
        with pytest.raises(ValueError):
            hiss.setDecimalSeparator('')  # Empty string
        
        with pytest.raises(ValueError):
            hiss.setDecimalSeparator('too_long')  # More than one character
        
        with pytest.raises(ValueError):
            hiss.setDecimalSeparator(123)  # Not a string
...
def test_cursor_setinputsizes_basic(db_connection):
    ...
    cursor.setinputsizes([
        (hiss.SQL_WVARCHAR, 100, 0),
        (hiss.SQL_INTEGER, 0, 0)
    ])
...
def test_cursor_setinputsizes_override_inference(db_connection):
    ...
    cursor.setinputsizes([
        (hiss.SQL_SMALLINT, 5, 0),  # Use valid precision for SMALLINT
        (hiss.SQL_WVARCHAR, 8000, 0)  # Force short string to NVARCHAR(MAX)
    ])
...
def test_setinputsizes_parameter_count_mismatch_more(db_connection):
    ...
    cursor.setinputsizes([
        (hiss.SQL_INTEGER, 0, 0),
        (hiss.SQL_WVARCHAR, 100, 0),
        (hiss.SQL_FLOAT, 0, 0)  # Extra parameter type
    ])
...
def test_setinputsizes_sql_injection_protection(db_connection):
    ...
    cursor.setinputsizes([(hiss.SQL_VARCHAR, 100, 0)])
